export default function shuffle(txt) {
  const chars =
   ...
   'bcdeÃÃ©fghiÃjklmn0123456789oÃ$@B%8&WM#*oahk0123456789bdpqwmZO0QLCJUYXzc0123456789vunxrjft1ilI';
  ...
  let str;
  let c = 0;
  const almostIndex = Math.floor(Math.random() * (width/2));
  let almostText;
  let almostChar = finalText.charAt(almostIndex);

  while (almostChar == finalText.charAt(almostIndex)) {
    almostChar = chars.charAt(Math.floor(Math.random() * chars.length));
  }
